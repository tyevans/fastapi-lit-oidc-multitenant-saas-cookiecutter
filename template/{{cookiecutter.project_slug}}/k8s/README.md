...
This directory contains Kubernetes manifests for deploying {{ cookiecutter.project_name }} using [Kustomize](https://kustomize.io/).
...
image: ghcr.io/{{ cookiecutter.github_username }}/{{ cookiecutter.project_slug }}-backend:latest

# k8s/base/frontend-deployment.yaml
image: ghcr.io/{{ cookiecutter.github_username }}/{{ cookiecutter.project_slug }}-frontend:latest
```
...
# Check pod status
kubectl get pods -n {{ cookiecutter.project_slug }} -w

# Check deployment rollout
kubectl rollout status deployment/backend -n {{ cookiecutter.project_slug }}
kubectl rollout status deployment/frontend -n {{ cookiecutter.project_slug }}
...
| `API_V1_PREFIX` | API path prefix | `{{ cookiecutter.backend_api_prefix }}` |
| `OAUTH_ISSUER_URL` | Keycloak realm URL | Update for your deployment |
{% if cookiecutter.include_observability == "yes" %}| `OTEL_SERVICE_NAME` | OpenTelemetry service name | `backend` |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | OTLP collector endpoint | `http://tempo.observability:4317` |{% endif %}
...
{% if cookiecutter.include_sentry == "yes" %}| `SENTRY_DSN` | Sentry error tracking DSN | `https://key@sentry.io/project` |{% endif %}
...
namespace: {{ cookiecutter.project_slug }}-staging
...
# Override image tags
images:
  - name: ghcr.io/{{ cookiecutter.github_username }}/{{ cookiecutter.project_slug }}-backend
    newTag: staging-abc123
  - name: ghcr.io/{{ cookiecutter.github_username }}/{{ cookiecutter.project_slug }}-frontend
    newTag: staging-abc123
...
kubectl set image deployment/backend \
  backend=ghcr.io/{{ cookiecutter.github_username }}/{{ cookiecutter.project_slug }}-backend:v1.2.3 \
...
kubectl rollout undo deployment/backend --to-revision=2 -n {{ cookiecutter.project_slug }}
```
...
```bash
# Manual scaling
kubectl scale deployment/backend --replicas=5 -n {{ cookiecutter.project_slug }}
...
```bash
# Backend logs
kubectl logs -n {{ cookiecutter.project_slug }} -l app.kubernetes.io/component=backend -f

# Frontend logs
kubectl logs -n {{ cookiecutter.project_slug }} -l app.kubernetes.io/component=frontend -f

# Specific pod logs
kubectl logs -n {{ cookiecutter.project_slug }} backend-xxxxx-xxxxx -f
```
...
kubectl port-forward -n {{ cookiecutter.project_slug }} svc/frontend 8080:80
...
kubectl exec -it -n {{ cookiecutter.project_slug }} deploy/backend -- /bin/sh

# Run database migrations
kubectl exec -it -n {{ cookiecutter.project_slug }} deploy/backend -- \
...
# Check pod logs
kubectl logs -n {{ cookiecutter.project_slug }} <pod-name> --previous
...
kubectl exec -it -n {{ cookiecutter.project_slug }} deploy/backend -- \
  curl -v http://localhost:8000{{ cookiecutter.backend_api_prefix }}/health

kubectl exec -it -n {{ cookiecutter.project_slug }} deploy/frontend -- \
...
kubectl get certificate -n {{ cookiecutter.project_slug }}
kubectl describe certificate {{ cookiecutter.project_slug }}-tls -n {{ cookiecutter.project_slug }}
```
...
kubectl exec -it -n {{ cookiecutter.project_slug }} deploy/backend -- env | grep DATABASE
```
   ...
   spec:
     ...
     data:
       - secretKey: DATABASE_URL
         remoteRef:
           key: {{ cookiecutter.project_slug }}/database-url
   ```
