...
```bash
docker build --target development -t {{ cookiecutter.project_slug }}-frontend:dev .
docker run -p 5173:5173 \
  -e VITE_API_URL=http://localhost:8000 \
  -v $(pwd):/app \
  -v /app/node_modules \
  {{ cookiecutter.project_slug }}-frontend:dev
```
...
```bash
docker build --target production -t {{ cookiecutter.project_slug }}-frontend:prod .
docker run -p 80:80 {{ cookiecutter.project_slug }}-frontend:prod
```
