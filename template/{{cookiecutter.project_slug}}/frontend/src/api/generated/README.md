...
const config = new Configuration({
  basePath: import.meta.env.VITE_API_URL || 'http://localhost:{{ cookiecutter.backend_port }}',
})
...
1. **Phase 1**: Use both clients side-by-side
   ...
   const response = await apiClient.get<Todo[]>('{{ cookiecutter.backend_api_prefix }}/todos')
...
```
Error: Backend is not running at http://localhost:{{ cookiecutter.backend_port }}
```
