...
def keycloak_oauth_client():
    ...
    return OAuthClient(
        ...
        issuer_url="http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}",
...
class TestOAuthClientIntegration:
    ...
    async def test_oidc_discovery_real_keycloak(self, keycloak_oauth_client):
        ...
        try:
            ...
            assert config.issuer == "http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}"
    ...
    async def test_authorization_url_generation_real_keycloak(self, keycloak_oauth_client):
        ...
        try:
            ...
            assert url.startswith("http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/auth")
            assert "response_type=code" in url
            assert "client_id={{ cookiecutter.keycloak_backend_client_id }}" in url
