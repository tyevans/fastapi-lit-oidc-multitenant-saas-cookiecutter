...
_rls_test_url = settings.DATABASE_URL.replace("{{ cookiecutter.postgres_user }}:{{ cookiecutter.postgres_password }}", "{{ cookiecutter.postgres_app_user }}:{{ cookiecutter.postgres_app_password }}")
...
async def rls_session(tenant_a, tenant_b, users_tenant_a, users_tenant_b):
    """
    Create a session using {{ cookiecutter.postgres_app_user }} (NO BYPASSRLS) for RLS testing.
