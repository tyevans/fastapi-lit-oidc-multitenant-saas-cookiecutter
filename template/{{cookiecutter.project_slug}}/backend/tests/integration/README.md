...
1. **Keycloak** (http://localhost:8080)
   ...
   - Client: `{{ cookiecutter.keycloak_backend_client_id }}` (confidential)
   - Test users configured (from TASK-007)

2. **Redis** (redis://localhost:{{ cookiecutter.redis_port }})
   - For JWKS caching
   - For token revocation blacklist
   - For rate limiting state

3. **PostgreSQL** (localhost:{{ cookiecutter.postgres_port }})
   - Database: `{{ cookiecutter.postgres_db }}`
...
os.environ["OAUTH_ISSUER_URL"] = "http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}"
os.environ["REDIS_URL"] = "redis://default:{{ cookiecutter.redis_password }}@localhost:{{ cookiecutter.redis_port }}/0"
os.environ["DATABASE_URL"] = "postgresql+asyncpg://{{ cookiecutter.postgres_app_user }}:{{ cookiecutter.postgres_app_password }}@localhost:{{ cookiecutter.postgres_port }}/{{ cookiecutter.postgres_db }}"
os.environ["OAUTH_AUDIENCE"] = "account"  # Keycloak default
```

**Why**: Backend normally uses Docker hostnames (keycloak:8080, redis:{{ cookiecutter.redis_port }}) but integration tests run outside Docker.
...
- Realm: {{ cookiecutter.keycloak_realm_name }}
- Client ID: {{ cookiecutter.keycloak_backend_client_id }}
- Client Secret: {{ cookiecutter.keycloak_backend_client_id }}-secret
...
async def test_jwks_fetch(redis_client_integration):
    ...
    jwks = await jwks_client.get_jwks("http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}")
    assert "keys" in jwks
...
```bash
curl http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/.well-known/openid-configuration
```

### Get Token Manually

```bash
curl -X POST http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/token \
...
```bash
redis-cli -h localhost -p 6379 -a {{ cookiecutter.redis_password }}
> KEYS jwks:*
> GET jwks:http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}
```
