"""
Test Row-Level Security (RLS) enforcement with {{ cookiecutter.postgres_app_user }}.

This test verifies that:
1. {{ cookiecutter.postgres_app_user }} (NO BYPASSRLS) respects RLS policies
...
APP_USER_URL = "postgresql://{{ cookiecutter.postgres_app_user }}:{{ cookiecutter.postgres_app_password }}@localhost:{{ cookiecutter.postgres_port }}/{{ cookiecutter.postgres_db }}"


@pytest.fixture(scope="module")
def app_user_engine():
    """Create database engine with {{ cookiecutter.postgres_app_user }} (NO BYPASSRLS)."""
...
def app_user_session(app_user_engine):
    """Create database session with {{ cookiecutter.postgres_app_user }}."""
...
class TestRLSEnforcementWithAppUser:
    """Test RLS enforcement with {{ cookiecutter.postgres_app_user }} (NO BYPASSRLS)."""

    def test_app_user_cannot_bypass_rls(self, app_user_session):
        """
        Test that {{ cookiecutter.postgres_app_user }} does NOT have BYPASSRLS privilege.
        ...
        result = app_user_session.execute(
            text("""
                SELECT rolbypassrls
                FROM pg_roles
                WHERE rolname = '{{ cookiecutter.postgres_app_user }}'
            """)
        ).fetchone()

        assert result is not None, "{{ cookiecutter.postgres_app_user }} role not found"
        assert result[0] is False, "CRITICAL: {{ cookiecutter.postgres_app_user }} has BYPASSRLS privilege!"
    ...
    def test_migration_user_has_bypassrls(self):
        """
        Test that {{ cookiecutter.postgres_migration_user }} DOES have BYPASSRLS privilege.

        This verifies that migrations can still run with administrative privileges.
        """
        migration_user_url = "postgresql://{{ cookiecutter.postgres_migration_user }}:{{ cookiecutter.postgres_migration_password }}@localhost:{{ cookiecutter.postgres_port }}/{{ cookiecutter.postgres_db }}"
        ...
        try:
            result = session.execute(
                text("""
                    SELECT rolbypassrls
                    FROM pg_roles
                    WHERE rolname = '{{ cookiecutter.postgres_migration_user }}'
                """)
            ).fetchone()

            assert result is not None, "{{ cookiecutter.postgres_migration_user }} role not found"
            assert result[0] is True, "{{ cookiecutter.postgres_migration_user }} should have BYPASSRLS privilege"
