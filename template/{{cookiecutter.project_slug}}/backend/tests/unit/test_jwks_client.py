...
def mock_oidc_discovery():
    ...
    return {
        "issuer": "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}",
        "authorization_endpoint": "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/auth",
        "token_endpoint": "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/token",
        "jwks_uri": "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/certs",
    }
...
async def test_fetch_jwks_success(mock_redis, mock_jwks, mock_oidc_discovery):
    ...
    with patch.object(client._http_client, "get") as mock_get:
        ...
        # Fetch JWKS
        result = await client.get_jwks("http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}")
        ...
        mock_get.assert_any_call(
            "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/.well-known/openid-configuration"
        )
        mock_get.assert_any_call(
            "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/certs"
        )
...
async def test_jwks_cache_hit(mock_redis, mock_jwks):
    ...
    with patch.object(client._http_client, "get") as mock_get:
        ...
        mock_redis.get.assert_called_once_with("jwks:http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}")
...
async def test_jwks_cache_miss(mock_redis, mock_jwks, mock_oidc_discovery):
    ...
    with patch.object(client._http_client, "get") as mock_get:
        ...
        mock_redis.setex.assert_called_once_with(
            "jwks:http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}",
            3600,
...
async def test_force_refresh_bypasses_cache(mock_redis, mock_jwks, mock_oidc_discovery):
    ...
    with patch.object(client._http_client, "get") as mock_get:
        ...
        result = await client.get_jwks(
            "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}", force_refresh=True
        )
...
async def test_get_signing_key_found(mock_redis, mock_jwks):
    ...
    key = await client.get_signing_key(
        "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}", "test-key-1"
    )
    ...
    key2 = await client.get_signing_key(
        "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}", "test-key-2"
    )
...
async def test_get_signing_key_not_found(mock_redis, mock_jwks):
    ...
    key = await client.get_signing_key(
        "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}", "nonexistent-key"
    )
...
async def test_invalid_jwks_format(mock_redis, mock_oidc_discovery):
    ...
    with patch.object(client._http_client, "get") as mock_get:
        ...
        with pytest.raises(ValueError, match="Invalid JWKS format"):
            await client.get_jwks("http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}")
...
async def test_multi_issuer_support(mock_redis, mock_jwks, mock_oidc_discovery):
    ...
    issuer1 = "http://keycloak:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}"
...
async def test_invalid_json_in_cache(mock_redis, mock_jwks, mock_oidc_discovery):
    ...
    mock_redis.get = AsyncMock(return_value="invalid json {% raw %}{{{{% endraw %}")
