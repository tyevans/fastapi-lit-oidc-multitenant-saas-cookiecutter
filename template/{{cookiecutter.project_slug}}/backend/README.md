...
FastAPI-based backend service for {{ cookiecutter.project_name }}.
...
```bash
docker build --target development -t {{ cookiecutter.project_slug }}-backend:dev .
docker run -p 8000:8000 -v $(pwd):/app {{ cookiecutter.project_slug }}-backend:dev
```
...
- `APP_NAME`: Application name (default: "{{ cookiecutter.project_name }}")
