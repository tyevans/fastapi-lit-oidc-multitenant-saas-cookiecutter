#!/usr/bin/env bash
# {{ cookiecutter.project_name }} - Docker Development Helper Script
...
show_usage() {
    cat << EOF
{{ cookiecutter.project_name }} - Docker Development Helper
...
cmd_up() {
    ...
    echo "  - Keycloak:  http://localhost:{{ cookiecutter.keycloak_port }}"
    echo "  - PostgreSQL: localhost:{{ cookiecutter.postgres_port }}"
    echo "  - Redis:     localhost:{{ cookiecutter.redis_port }}"
}
