...
1. Check database connections:
   ```bash
   docker compose exec postgres psql -U {{ cookiecutter.postgres_user }} -c "SELECT * FROM pg_stat_activity"
   ```
...
| Variable | Description | Default |
|----------|-------------|---------|
| `TOKEN_URL` | OAuth token endpoint | `http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/token` |
| `CLIENT_ID` | OAuth client ID | `{{ cookiecutter.project_slug }}-app` |
...
curl -X POST http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/token \
  -d "grant_type=password&client_id={{ cookiecutter.project_slug }}-app&username=alice@example.com&password=password123"
```
