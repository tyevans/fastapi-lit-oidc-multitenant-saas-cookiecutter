...
curl http://localhost:{{ cookiecutter.backend_port }}{{ cookiecutter.backend_api_prefix }}/health
```
...
docker run --rm -i --network host \
    -v $(pwd)/tests/load:/tests \
    -e BASE_URL=http://localhost:{{ cookiecutter.backend_port }} \
...
```bash
# Get a token
TOKEN=$(curl -s -X POST http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/token \
    -d "grant_type=password" \
    -d "client_id={{ cookiecutter.project_slug }}-app" \
...
export TOKEN_URL="http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/token"
export CLIENT_ID="{{ cookiecutter.project_slug }}-app"
  ...
  performance:
    ...
    steps:
      ...
      - name: Wait for services
        run: |
          timeout 120 bash -c 'until curl -f http://localhost:{{ cookiecutter.backend_port }}{{ cookiecutter.backend_api_prefix }}/health; do sleep 2; done'
...
curl http://localhost:{{ cookiecutter.keycloak_port }}
...
{% if cookiecutter.include_observability == 'yes' %}- [Grafana Dashboards](../../observability/grafana/) - Visualize test results{% endif %}
...
open http://localhost:{{ cookiecutter.keycloak_port }}/admin
# Login: admin / admin
...
# Or manually via API
for i in {1..100}; do
    curl -X POST http://localhost:{{ cookiecutter.backend_port }}{{ cookiecutter.backend_api_prefix }}/todos \
