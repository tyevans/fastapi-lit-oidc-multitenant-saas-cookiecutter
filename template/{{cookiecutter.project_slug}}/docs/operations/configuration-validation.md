...
This guide covers how to validate your {{ cookiecutter.project_name }} configuration and troubleshoot common configuration issues.
...
kubectl get configmap {{ cookiecutter.project_slug }}-config -n {{ cookiecutter.project_slug }}

# View ConfigMap contents
kubectl describe configmap {{ cookiecutter.project_slug }}-config -n {{ cookiecutter.project_slug }}

# Check pod is using ConfigMap
kubectl get pod <pod-name> -n {{ cookiecutter.project_slug }} -o yaml | grep -A 10 envFrom
```
...
kubectl exec -n {{ cookiecutter.project_slug }} <pod-name> -- env | sort

# Check specific variable
kubectl exec -n {{ cookiecutter.project_slug }} <pod-name> -- printenv DATABASE_URL
```
...
curl http://localhost:8000{{ cookiecutter.backend_api_prefix }}/health/ready
