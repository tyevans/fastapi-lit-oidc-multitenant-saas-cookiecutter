...
|----------|-----|-----|-----|------------|-------|
| `GET {{ cookiecutter.backend_api_prefix }}/health` | < 10ms | < 25ms | < 50ms | 1000+ | Simple health check |
| `GET {{ cookiecutter.backend_api_prefix }}/health/ready` | < 20ms | < 50ms | < 100ms | 500+ | Includes DB/Redis check |
| `GET {{ cookiecutter.backend_api_prefix }}/health/live` | < 5ms | < 15ms | < 30ms | 1000+ | Minimal liveness probe |

### Authentication Endpoints

| Endpoint | P50 | P95 | P99 | Target RPS | Notes |
|----------|-----|-----|-----|------------|-------|
| `POST {{ cookiecutter.backend_api_prefix }}/auth/token` | < 100ms | < 200ms | < 400ms | 50+ | Token exchange (Keycloak round-trip) |
| `POST {{ cookiecutter.backend_api_prefix }}/auth/refresh` | < 50ms | < 100ms | < 200ms | 100+ | Token refresh |
| `GET {{ cookiecutter.backend_api_prefix }}/auth/me` | < 30ms | < 75ms | < 150ms | 200+ | JWT validation only |
| `POST {{ cookiecutter.backend_api_prefix }}/auth/logout` | < 50ms | < 100ms | < 200ms | 100+ | Token revocation |

### API Endpoints (CRUD Operations)

| Endpoint | P50 | P95 | P99 | Target RPS | Notes |
|----------|-----|-----|-----|------------|-------|
| `GET {{ cookiecutter.backend_api_prefix }}/todos` | < 50ms | < 100ms | < 200ms | 200+ | List with pagination |
| `POST {{ cookiecutter.backend_api_prefix }}/todos` | < 75ms | < 150ms | < 300ms | 100+ | Create operation |
| `GET {{ cookiecutter.backend_api_prefix }}/todos/{id}` | < 30ms | < 75ms | < 150ms | 300+ | Single item fetch |
| `PUT {{ cookiecutter.backend_api_prefix }}/todos/{id}` | < 75ms | < 150ms | < 300ms | 100+ | Update operation |
| `DELETE {{ cookiecutter.backend_api_prefix }}/todos/{id}` | < 50ms | < 100ms | < 200ms | 150+ | Delete operation |
...
|----------|-----|-----|-----|-----|-------|
| `GET {{ cookiecutter.backend_api_prefix }}/health` | _Xms_ | _Xms_ | _Xms_ | _X_ | |
| `GET {{ cookiecutter.backend_api_prefix }}/todos` | _Xms_ | _Xms_ | _Xms_ | _X_ | With N todos |
| `POST {{ cookiecutter.backend_api_prefix }}/todos` | _Xms_ | _Xms_ | _Xms_ | _X_ | |
...
```
{% else %}
Configure your monitoring system with these thresholds:

| Alert | Metric | Warning | Critical | Duration |
|-------|--------|---------|----------|----------|
| High Latency | P95 response time | > 500ms | > 1000ms | 5 min |
| Error Rate | 5xx rate | > 1% | > 5% | 5 min |
| CPU Usage | Container CPU | > 70% | > 85% | 10 min |
| Memory Usage | Container memory | > 75% | > 90% | 10 min |
{% endif %}
...
{% if cookiecutter.include_observability == 'yes' %}- [Prometheus Alerting](../../observability/prometheus/alerts/) - Alert configuration{% endif %}
