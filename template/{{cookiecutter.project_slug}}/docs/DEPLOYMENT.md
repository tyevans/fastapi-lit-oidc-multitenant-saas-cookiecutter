...
This document describes the deployment process for {{ cookiecutter.project_name }} using GitHub Actions and Kubernetes.
...
docker pull ghcr.io/{{ cookiecutter.github_username }}/{{ cookiecutter.project_slug }}-backend:TAG

# Verify pull secret (if using private registry)
kubectl get secret -n {{ cookiecutter.project_slug }}
```

**CrashLoopBackOff**:
```bash
# Check pod logs
kubectl logs -n {{ cookiecutter.project_slug }} -l app.kubernetes.io/component=backend --previous
...
kubectl get deployment backend -n {{ cookiecutter.project_slug }} -o yaml | grep -A5 livenessProbe
```
...
kubectl get deployment -n {{ cookiecutter.project_slug }}
...
sum(rate(http_requests_total{namespace="{{ cookiecutter.project_slug }}", status!~"5.."}[5m])) /
sum(rate(http_requests_total{namespace="{{ cookiecutter.project_slug }}"}[5m]))
```
