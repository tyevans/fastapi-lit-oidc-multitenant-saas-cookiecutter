...
# http://localhost:{{ cookiecutter.prometheus_port }}/graph?g0.expr=histogram_quantile(0.95,sum(rate(http_request_duration_seconds_bucket[5m]))by(le))
```
...
docker inspect backend --format='{% raw %}{{.HostConfig.CpuQuota}}{% endraw %}'
```
...
time curl -s http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/.well-known/openid-configuration > /dev/null
```
