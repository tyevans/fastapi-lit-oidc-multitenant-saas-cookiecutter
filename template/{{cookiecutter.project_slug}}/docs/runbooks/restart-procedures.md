...
{% if cookiecutter.include_observability == "yes" %}| Prometheus | `docker compose restart prometheus` | ~10s | None |
| Grafana | `docker compose restart grafana` | ~10s | prometheus, loki |
| Loki | `docker compose restart loki` | ~10s | None |
| Tempo | `docker compose restart tempo` | ~10s | None |{% endif %}
...
curl -s http://localhost:{{ cookiecutter.backend_port }}/health | jq .
...
curl -s -o /dev/null -w "%{http_code}" http://localhost:{{ cookiecutter.frontend_port }}
...
FROM pg_stat_activity
WHERE datname = '{{ cookiecutter.project_slug }}'
...
docker compose exec postgres psql -U {{ cookiecutter.postgres_user }} -c "CHECKPOINT;"
...
# Verify targets
curl -s http://localhost:{{ cookiecutter.prometheus_port }}/api/v1/targets | jq '.data.activeTargets | length'
```
...
# Verify
curl -s -o /dev/null -w "%{http_code}" http://localhost:{{ cookiecutter.grafana_port }}/api/health
```
...
```bash
# Check exit code
docker inspect <container> --format='{% raw %}{{.State.ExitCode}}{% endraw %}'

# Check OOM
docker inspect <container> --format='{% raw %}{{.State.OOMKilled}}{% endraw %}'
