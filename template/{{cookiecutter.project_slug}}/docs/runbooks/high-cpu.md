...
| Item | Value |
|------|-------|
| **Dashboard** | http://localhost:{{ cookiecutter.grafana_port }}/d/infrastructure |
...
```bash
# View current limits
docker inspect backend --format='{% raw %}{{.HostConfig.CpuQuota}} {{.HostConfig.CpuPeriod}}{% endraw %}'
...
curl -s http://localhost:{{ cookiecutter.backend_port }}/debug/pprof/ 2>/dev/null || echo "Profiling not available"
```
...
curl -s http://localhost:{{ cookiecutter.backend_port }}/metrics | grep rate_limit
...
- [Infrastructure Dashboard](http://localhost:{{ cookiecutter.grafana_port }}/d/infrastructure)
