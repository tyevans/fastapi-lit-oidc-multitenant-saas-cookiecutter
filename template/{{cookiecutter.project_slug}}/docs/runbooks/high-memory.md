...
docker stats --no-stream --format "table {% raw %}{{.Name}}\t{{.MemUsage}}\t{{.MemPerc}}{% endraw %}"

# Detailed memory info
docker inspect backend --format='{% raw %}{{.HostConfig.Memory}}{% endraw %}'
```
...
curl -X POST http://localhost:{{ cookiecutter.backend_port }}/debug/gc
