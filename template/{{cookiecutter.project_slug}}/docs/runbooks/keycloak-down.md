...
| Item | Value |
|------|-------|
| **Dashboard** | http://localhost:{{ cookiecutter.grafana_port }}/d/keycloak/keycloak-overview |
| **Logs** | `{container="keycloak"}` |
| **Metrics** | `up{job="keycloak"}` |
| **Escalation** | Infrastructure team |
| **Admin Console** | http://localhost:{{ cookiecutter.keycloak_port }}/admin |
...
curl -s http://localhost:{{ cookiecutter.keycloak_port }}/health/ready

# Check if port is responding
curl -s -o /dev/null -w "%{http_code}" http://localhost:{{ cookiecutter.keycloak_port }}
```
...
# Check for OOM
docker inspect keycloak --format='{% raw %}{{.State.OOMKilled}} {{.State.ExitCode}}{% endraw %}'

# Check container health
docker inspect keycloak --format='{% raw %}{{.State.Health.Status}}{% endraw %}'
```
...
docker compose exec postgres psql -U {{ cookiecutter.postgres_user }} -d keycloak -c "\dt"
```
...
docker compose exec postgres psql -U {{ cookiecutter.postgres_user }} -c "SELECT 1"

# Check Keycloak database exists
docker compose exec postgres psql -U {{ cookiecutter.postgres_user }} -c "\l" | grep keycloak

# Create if missing
docker compose exec postgres psql -U {{ cookiecutter.postgres_user }} -c "CREATE DATABASE keycloak;"
```
...
```bash
# Health check
curl -s http://localhost:{{ cookiecutter.keycloak_port }}/health/ready | jq .

# Test realm discovery
curl -s http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/.well-known/openid-configuration | jq .issuer

# Test JWKS endpoint (used by backend)
curl -s http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/protocol/openid-connect/certs | jq .

# Test login page loads
curl -s -o /dev/null -w "%{http_code}" "http://localhost:{{ cookiecutter.keycloak_port }}/realms/{{ cookiecutter.keycloak_realm_name }}/account"
```
...
| Date | Author | Change |
|------|--------|--------|
| {{ cookiecutter._creation_date }} | {{ cookiecutter.author_name }} | Initial creation |
