...
| Item | Value |
|------|-------|
| **Dashboard** | http://localhost:{{ cookiecutter.grafana_port }}/d/prometheus |
...
curl -s http://localhost:{{ cookiecutter.prometheus_port }}/-/healthy

# Check Prometheus ready status
curl -s http://localhost:{{ cookiecutter.prometheus_port }}/-/ready
```
...
curl -s "http://localhost:{{ cookiecutter.prometheus_port }}/api/v1/query?query=prometheus_notifications_queue_length" | jq .

# Query dropped notifications
curl -s "http://localhost:{{ cookiecutter.prometheus_port }}/api/v1/query?query=prometheus_notifications_dropped_total" | jq .
```
...
curl -X POST http://localhost:{{ cookiecutter.prometheus_port }}/-/reload
...
docker inspect prometheus --format='{% raw %}{{.NetworkSettings.Networks}}{% endraw %}'
docker inspect alertmanager --format='{% raw %}{{.NetworkSettings.Networks}}{% endraw %}'
```
...
watch 'curl -s "http://localhost:{{ cookiecutter.prometheus_port }}/api/v1/query?query=prometheus_notifications_queue_length" | jq .data.result[].value[1]'
```
...
```bash
# List active alerts
curl -s http://localhost:{{ cookiecutter.prometheus_port }}/api/v1/alerts | jq .

# List firing alerts
curl -s "http://localhost:{{ cookiecutter.prometheus_port }}/api/v1/query?query=ALERTS{alertstate=\"firing\"}" | jq .
```
...
#!/bin/bash
alerts=$(curl -s http://localhost:{{ cookiecutter.prometheus_port }}/api/v1/alerts | jq -r '.data.alerts[] | select(.state == "firing") | .labels.alertname')
if [ -n "$alerts" ]; then
  echo "FIRING ALERTS:"
  echo "$alerts"
fi
...
- [Grafana Dashboard](http://localhost:{{ cookiecutter.grafana_port }}/d/prometheus)
