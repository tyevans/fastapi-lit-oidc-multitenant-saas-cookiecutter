...
- [ ] Security headers appear in API responses:
  ```bash
  curl -I http://localhost:{{ cookiecutter.backend_port }}/health
  ```
...
4. **Customize for your project:**
   ...
   image: ghcr.io/YOUR_ORG/{{ cookiecutter.project_slug }}-backend:latest

   # In frontend-deployment.yaml
   image: ghcr.io/YOUR_ORG/{{ cookiecutter.project_slug }}-frontend:latest
   ```
...
- [ ] Security headers present in responses:
  ```bash
  curl -I http://localhost:{{ cookiecutter.backend_port }}/health | grep -E "(X-Frame|X-Content|CSP|HSTS)"
  ```
...
docker pull ghcr.io/your-org/{{ cookiecutter.project_slug }}-backend:latest

# Verify image pull secrets
kubectl get secret regcred -n {{ cookiecutter.project_slug }} -o yaml
...
curl http://localhost:{{ cookiecutter.backend_port }}/openapi.json
...
4. **Community resources:**
   ...
   - Stack Overflow (tag: `{{ cookiecutter.project_slug }}`)
