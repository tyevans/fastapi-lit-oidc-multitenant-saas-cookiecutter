...
|----------|-------------|
| `GET {{ cookiecutter.backend_api_prefix }}/health` | Application health check with service status |
| `GET {{ cookiecutter.backend_api_prefix }}/health/ready` | Kubernetes readiness probe |
| `GET {{ cookiecutter.backend_api_prefix }}/health/live` | Kubernetes liveness probe |

#### Authentication Endpoints

| Endpoint | Description |
|----------|-------------|
| `POST {{ cookiecutter.backend_api_prefix }}/auth/token` | Exchange authorization code for access/refresh tokens |
| `POST {{ cookiecutter.backend_api_prefix }}/auth/refresh` | Refresh expired access token |
| `GET {{ cookiecutter.backend_api_prefix }}/auth/me` | Get current authenticated user information |
| `POST {{ cookiecutter.backend_api_prefix }}/auth/logout` | Revoke tokens and end session |

#### OAuth Endpoints

| Endpoint | Description |
|----------|-------------|
| `GET {{ cookiecutter.backend_api_prefix }}/oauth/authorize` | Initiate OAuth 2.0 authorization flow |
| `GET {{ cookiecutter.backend_api_prefix }}/oauth/callback` | OAuth callback handler |

#### Todo Endpoints (Demo Resource)

| Endpoint | Description |
|----------|-------------|
| `GET {{ cookiecutter.backend_api_prefix }}/todos` | List todos for current tenant (paginated) |
| `POST {{ cookiecutter.backend_api_prefix }}/todos` | Create a new todo |
| `GET {{ cookiecutter.backend_api_prefix }}/todos/{id}` | Get todo by ID |
| `PUT {{ cookiecutter.backend_api_prefix }}/todos/{id}` | Update a todo |
| `DELETE {{ cookiecutter.backend_api_prefix }}/todos/{id}` | Delete a todo |
...
{% if cookiecutter.include_observability == 'yes' %}- **Observability**: Prometheus metrics, Loki logging, Tempo tracing{% endif %}
